...
# Version:      5.83 BETA	DOES NOT WORK! REVERT TO 5.82
# Last Updated: 2021-01-12
# Changelog:
# - Attempts to fix why it always says use almost no energy. Redoing optimization eqns to match older code...
...
A = np.zeros((n,n))#np.zeros((2*n, n))
for i in range(0, n):
	#j = 2*i
	j = i
	A[j,i] = C_THERMAL * (timestep * c2)
	#A[j+1,i] = C_THERMAL * -(timestep * c2)
for k in range(0,n):
	#h = 2*k+2
	h = k+1
	while j<n:#j < 2*n-1:
		A[h,k] = A[h-1, k] + A[h-1, k] * (-timestep * c1)
		#A[h+1,k] = A[h-1,k]*-timestep*c1+A[h-1,k]
		j += 1 #2
...
objective = cp.Minimize(cp.sum(cc@E + cp.maximum(A@E + b, zero)))
print('\n')
print(objective)
print('\n')
